    ...
    (set, get) => ({
      ...
      fetchBalance: async () => {
        try {
          ...
          set({totalBalance:( new BigNumberInWei(balance.amount).toBase().toFixed(2))})
        } catch (error) {
      ...
      fetchTotalEarned: async () => {
        try {
          ...
          if (response) {
            ...
            set({
              totalEarned: (new BigNumberInWei(result.total_earned).toBase().toString())
            })
          }
        } catch (error) {
          ...
          if (response) {
            ...
            result.nft_maps.forEach((nft: any) => {
              nftsArray.push({
                ...
                airdrop: new BigNumberInWei(nft.airdrop).toBase().toNumber(), 
                token_uri: '',
              })
